...
class PlantDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = {
                    ...
                    'label2':torch.tensor(label//4).long(),
                }
        # category
        # data['label'] -= 1
...
class plModel(pl.LightningModule):
    def __init__(self, config):
        ...
        # [ssl_resnext101_32x4d, seresnext50_32x4d, efficientnet_b1_pruned, efficientnet_lite0, resnet34, tf_efficientnet_b0_ns, densenet121, tf_efficientnetv2_s_in21k, tf_mobilenetv3_large_075]
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat([out1, out2, out3, out4, out5, out6], 1)
        out = self.fc(x)
        clf_out = self.fc2(x)
    ...
    def training_step(self, train_batch, batch_idx):
        # pred, pred2 = self.forward(train_batch['be_img'], train_batch['af_img']).squeeze(1)
        pred, pred2 = self.forward(train_batch['be_img'], train_batch['af_img'])
        pred = pred.squeeze(1)
        # pred = self.forward(train_batch['be_img'], train_batch['af_img'])
        loss2 = self.criterion2(pred2, train_batch['label2'])
        ...
        self.log("train_loss2", loss2, on_step=True, prog_bar=True, logger=True)
        return {'loss':loss/2 + loss2/2, 'pred':pred.clone().detach().cpu(), 'label':train_batch['label'].clone().detach().cpu()}
    ...
    def validation_step(self, val_batch, batch_idx):
        # pred, pred2 = self.forward(val_batch['be_img'], val_batch['af_img']).squeeze(1)
        pred, pred2 = self.forward(val_batch['be_img'], val_batch['af_img'])
        pred = pred.squeeze(1)
        # pred = self.forward(val_batch['be_img'], val_batch['af_img'])
        
        loss2 = self.criterion2(pred2, val_batch['label2'])
        ...
        self.log("val_loss2", loss2, on_step=True, prog_bar=True, logger=True)
        return {"val_loss": loss/2+loss2/2, 'pred':pred.clone().detach().cpu(), 'label':val_batch['label'].clone().detach().cpu()}
